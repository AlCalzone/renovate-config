...
| Preset | Use for |
|---|---|
| `default` | Base for everything. Monthly window on the 1st (Europe/Berlin), 7 day minimum release age, `rangeStrategy: bump`, automerge for devDependency patch/minor, dependency patch, dependency minor when it fixes a vulnerability alert, and GitHub Actions minor/patch. Renovate merges itself once all checks on the branch pass (`platformAutomerge: false`), so repos without branch protection are safe too. TypeScript minor/major wait for dashboard approval. |
